	...
	public static void displayResult(List<ListResult> list) throws IOException {
		...
		if (list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				log((i + 1) + ".: Room type " + (i + 1) + ": (There are " + l.getCount()
						+ " rooms available for this room type)");
			...
			if (selection < list.size()) {
				if (null != list.get(selection)) {
					...
					log("You reserved " + l.getRoomName() + " for the dates between " + keywords.get(2) + " and "
							+ keywords.get(3));
					log((Integer) keywords.get(1) * l.getPrice() + "BAM has been taken from your account.");
				}
			} else {
				System.err.print("Wrong input. I am not giving you a second chance. Start from the beginning...");
			}
